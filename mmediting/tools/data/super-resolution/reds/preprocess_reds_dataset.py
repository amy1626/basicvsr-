...
def make_lmdb(mode, data_path, lmdb_path, batch=5000, compress_level=1):
    ...
    Args:
        mode (str): REDS dataset mode. Choices: ['train_sharp', 'train_blur',
            ...
            'train_blur': Blur frames for deblur task.去模糊任务的模糊帧
            'train_blur_comp': Blur and compressed frames for deblur and
                compression task.用于去模糊和压缩任务的模糊和压缩帧
            'train_sharp_bicubic': Bicubic downsampled sharp frames for SR
                task.用于 SR 任务的双三次下采样锐帧
            'train_blur_bicubic': Bicubic downsampled blur frames for SR task.
            用于 SR 任务的双三次下采样模糊帧
        data_path (str): Data path for reading images.读取图像的数据路径
        lmdb_path (str): Lmdb save path.
        batch (int): After processing batch images, lmdb commits.
            Default: 5000.处理批量图像后，lmdb 提交
        compress_level (int): Compress level when encoding images. Default: 1.
        编码图像时的压缩级别。默认值：1。
    """

    print(f'Create lmdb for {data_path}, save to {lmdb_path}...')
    if mode in ['train_sharp', 'train_blur', 'train_blur_comp','test_blur']:
...
def generate_anno_file(root_path, file_name='meta_info_REDS_GT.txt'):
    ...
    with open(txt_file, 'w') as f:
        ...
        for i in range(270):
            for j in range(100):
                f.write(f'{i:03d}/{j:08d}.png (1080, 1920, 3)\n')
...
'''Vid4的anno
        for i in range(1,42):
            f.write(f'calendar/{i:03d}.png (576, 720, 3)\n')
        for i in range(1,35):
            f.write(f'city/{i:03d}.png (576, 704, 3)\n')
        for i in range(1,50):
            f.write(f'foliage/{i:03d}.png (480, 720, 3)\n')
        for i in range(1,48):
            f.write(f'walk/{i:03d}.png (480, 720, 3)\n')
'''
def unzip(zip_path):
...
if __name__ == '__main__':
    ...
    # for a in train_folders:
        # print(a)
    #print("args.make_lmdb：",args.make_lmdb)
    # create lmdb file
    if args.make_lmdb:
        for train_folder in train_folders:
