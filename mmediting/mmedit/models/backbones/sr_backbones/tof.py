...
class TOFlow(nn.Module):
    ...
    def forward(self, lrs):
        ...
        lrs = self.normalize(lrs.view(-1, 3, h, w))#归一化
        lrs = lrs.view(num_batches, num_lrs, 3, h, w)

        lr_ref = lrs[:, self.ref_idx, :, :, :]#参考帧，中间帧
        lr_aligned = []#对齐后的lr
        for i in range(7):  # 7 frames
            if i == self.ref_idx:
                ...
                lr_supp = lrs[:, i, :, :, :]
                flow = self.spynet(lr_ref, lr_supp)#计算光流
                lr_aligned.append(flow_warp(lr_supp, flow.permute(0, 2, 3, 1)))#对齐操作
