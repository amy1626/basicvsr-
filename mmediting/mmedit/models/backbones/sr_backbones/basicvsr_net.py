...
class BasicVSRNet(nn.Module):
    ...
    def __init__(self, mid_channels=64, num_blocks=30, spynet_pretrained=None):
        ...
        #PixelShufflePack(in_channels, out_channels, scale_factor, upsample_kernel)
    ...
    def forward(self, lrs):
        ...
        for i in range(0, t):#正向
            lr_curr = lrs[:, i, :, :, :]    #当前帧
            if i > 0:  # no warping required for the first timestep
                if flows_forward is not None:
                    flow = flows_forward[:, i - 1, :, :, :]
                else:#如果flows_forward为None，也就是说输入图像序列为镜像序列
                    flow = flows_backward[:, -i, :, :, :]
                feat_prop = flow_warp(feat_prop, flow.permute(0, 2, 3, 1))
