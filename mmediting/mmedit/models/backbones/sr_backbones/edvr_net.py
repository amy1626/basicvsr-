...
class ModulatedDCNPack(ModulatedDeformConv2d):
    ...
    官方DCN生成的偏移量和掩码  与  前文不同，
    此ModulateDCNPack采用另一个不同的特征来生成偏移量和掩码

    Args:
        in_channels (int): Same as nn.Conv2d. 输入通道
        out_channels (int): Same as nn.Conv2d.输出通道
        kernel_size (int or tuple[int]): Same as nn.Conv2d.核的大小
        stride (int or tuple[int]): Same as nn.Conv2d.步长
        padding (int or tuple[int]): Same as nn.Conv2d.
        dilation (int or tuple[int]): Same as nn.Conv2d.膨胀卷积
        groups (int): Same as nn.Conv2d. 将输入数据分组
        要求in_channels核out_channels必须都可以被groups整除。
...
class EDVRNet(nn.Module):
    ...
    def __init__(self,
        ...
        self.feature_extraction = make_layer(#make_layer就是一个循环堆叠ResidualBlockNoBN的过程
            ResidualBlockNoBN,
    ...
    def forward(self, x):
        ...
        """
        n, t, c, h, w = x.size()#x:torch.Size([1, 5, 3, 180, 320])
        ...
        # print('aligned_feat:',aligned_feat.size())
        # aligned_feat: torch.Size([1, 5, 64, 180, 320])
        ...
        out += base
        #print('out:',out.size())#out:torch.Size([1, 3, 720, 1280])
        return out
    ...
    def init_weights(self, pretrained=None, strict=True):
        ...
        Args:
            ...
            预训练权重的路径。如果给定 None，则不会加载预训练的权重。 默认为无
            strict (bool, optional): Whether strictly load the pretrained model.
                Defaults to True.是否严格加载预训练模型。
        """
        if isinstance(pretrained, str):#是否有与训练的模型
