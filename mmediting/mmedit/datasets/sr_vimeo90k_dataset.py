...
class SRVimeo90KDataset(BaseSRDataset):
    ...
    def load_annotations(self):
        ...
        #自己数据集适应vimeo90k，不知道是否序列中图像全用于训练
        for key in keys:
            folder=key
            lq_paths=[]
            for i in frame_index_list:
                lq_paths.append(
                    osp.join(self.lq_folder,folder,f'{i:08d}.png'))
                if i==self.num_input_frames//2:
                    gt_paths = [osp.join(self.gt_folder, folder,f'{i:08d}.png')]
            data_infos.append(
                dict(lq_path=lq_paths, gt_path=gt_paths, key=key))
