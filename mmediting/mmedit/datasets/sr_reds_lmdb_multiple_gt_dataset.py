...
class SRREDSLmdbMultipleGTDataset(BaseSRDataset):
    ...
    def load_annotations(self):
        ...
        keys = [f'{i:03d}' for i in range(0, 30)]

        if self.val_partition == 'REDS4':
            val_partition = ['000', '011', '015', '020']
            #val_partition = ['000', '001', '002','003']
        ...
        for key in keys:
            data_infos.append(
                dict(
                    ...
                    sequence_length=30,  # REDS has 100 frames for each clip
                    num_input_frames=self.num_input_frames))
        #print('num_input_frames:',self.num_input_frames)   100
        return data_infos
