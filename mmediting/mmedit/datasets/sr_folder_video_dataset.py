...
class SRFolderVideoDataset(BaseSRDataset):
    ...
    该数据集采用注释文件指定训练或测试中使用的序列。 
    如果没有提供注释文件，则假定根目录下的所有视频序列都用于训练或测试。

    In the annotation file (.txt), each line contains:
    anno文件，每行包括，图像名称（不包括扩展名）、该序列共有的帧数量
    ...
    Args:
        ...
        ann_file (str): The path to the annotation file. If None, we assume
            that all sequences in the folder is used. Default: None.anno文件
        filename_tmpl (str): Template for each filename. Note that the每个文件名的模板。 
            template excludes the file extension. Default: '{:08d}'.请注意，模板不包括文件扩展名。
        metric_average_mode (str): The way to compute the average metric.计算平均度量的方法
            If 'clip', we first compute an average value for each clip, and，    若clip，先计算每个clip的平均值，再将不同clip的再平均
            then average the values from different clips. If 'all', we  
            compute the average of all frames. Default: 'clip'. 若all，则计算所有帧的平均值。
        test_mode (bool): Store `True` when building test dataset.是否为测试
            Default: `True`.
    """
